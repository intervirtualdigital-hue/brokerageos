...
export default function ListingDetailPage() {
    ...
    const deal = deals?.[0];
    const ndaSigned = deal?.ndaSigned ?? false;
            ...
            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div>
                    <div className="flex items-center gap-3 mb-2">
                        <StatusBadge status={ndaSigned ? 'success' : 'warning'}>
                            {deal ? deal.stage.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()) : 'Under LOI'}
                        </StatusBadge>
                        <span className="text-sm font-mono text-gray-500">ID: {deal?.id?.slice(0, 12) ?? 'L-2023-884'}</span>
                    </div>
                    <h2 className="text-3xl font-serif font-bold text-white mb-2">
                        {deal?.title ?? 'Project AURA: Returns Management SaaS'}
                    </h2>
                    <div className="flex items-center gap-4 text-sm text-gray-400">
                        <div className="flex items-center gap-1"><MapPin className="h-4 w-4" /> Austin, TX (Remote)</div>
                        <div className="flex items-center gap-1">
                            <DollarSign className="h-4 w-4" />
                            Asking: {deal?.askingPrice ? `$${(deal.askingPrice / 1_000_000).toFixed(1)}M` : '$4.2M'}
                        </div>
                    </div>
                </div>
                {!ndaSigned ? (
                    ...
                    <Button size="lg" onClick={() => navigate('/buy/data-room')}>
                        Enter Data Room
                    </Button>
                )}
            </div>
            ...
            <Tabs defaultValue="overview" className="w-full">
                <TabsList className="bg-white/5 border border-white/10 w-full justify-start h-auto p-1">
                    ...
                    <TabsTrigger value="financials">
                        Financials {!ndaSigned && <Lock className="ml-1.5 h-3 w-3 opacity-50" />}
                    </TabsTrigger>
                    <TabsTrigger value="documents">
                        Documents {!ndaSigned && <Lock className="ml-1.5 h-3 w-3 opacity-50" />}
                    </TabsTrigger>
                </TabsList>
                ...
                <TabsContent value="overview" className="mt-6 space-y-6">
                    <Card>
                        ...
                        <CardContent className="space-y-4 text-gray-300">
                            <p>
                                {deal?.title ?? 'Project AURA'} is a leading Returns Management System (RMS) specifically designed for D2C e-commerce brands on Shopify Plus.
                            ...
                            {deal?.valuation && (
                                <div className="mt-4 p-4 rounded-lg bg-accent/10 border border-accent/20">
                                    <p className="text-sm text-accent font-medium">
                                        Estimated Valuation: ${(deal.valuation / 1_000_000).toFixed(1)}M
                                    </p>
                                </div>
                            )}
                        </CardContent>
                    ...
                    {!ndaSigned ? (
                        ...
                        <Card>
                            ...
                            <CardContent className="text-gray-300">
                                <p>Financial data is now accessible. View detailed P&L, balance sheets, and cashflow analysis in the Data Room.</p>
                                <Button className="mt-4" onClick={() => navigate('/buy/data-room')}>Open Data Room</Button>
                            </CardContent>
                        </Card>
                    )}
                </TabsContent>
                ...
                <TabsContent value="documents" className="mt-6 relative">
                    {!ndaSigned ? (
                        ...
                        <Card>
                            <CardHeader><CardTitle>Deal Documents</CardTitle></CardHeader>
                            <CardContent className="text-gray-300">
                                <p>Documents are now unlocked. Access the full Data Room for contracts, tax returns, and IP documentation.</p>
                                <Button className="mt-4" onClick={() => navigate('/buy/data-room')}>Open Data Room</Button>
                            </CardContent>
                        </Card>
                    )}
